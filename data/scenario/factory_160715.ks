...
//add choice buttons		
	$('#choice-btn-wrapper').append('<button class="choice choice-a" value="0"><img src="./data/image/buttons/factory_choice_btn.png"><span class="choice_name">'+f.sim.state.choice[0]+'</span></button><button class="choice choice-b" value="1"><img src="./data/image/buttons/factory_choice_btn.png"><span class="choice_name">'+f.sim.state.choice[1]+'</span></button><button class="choice choice-c" value="2"><img src="./data/image/buttons/factory_choice_btn.png"><span class="choice_name">'+f.sim.state.choice[2]+'</span></button>');
...
//Add scenario title
	$('#tyrano_base').append('<div id="scenario-title"><h1>生産にバラツキのあるラインを発見せよ！</h1></div><img id="scenario-title-img" src="./data/image/candy.png"/>');	
...
[jump  storage="factory.ks"  target="*scenario-factory"  ]


*scenario-factory

[wait  time="300"  ]
[bg  time="500"  method="crossfade"  storage="factory_bg1.png"  ]

[layopt layer=0 visible=true]
[mtext name="mtext" text="菓子製造工場" x=360 y=240 size=36 time=500 in_effect="fadeInLeft" in_sync="true" out_effect="fadeOutRight" out_sync="true"]

[chara_show  name="dummy"  time="500" top="120" left="160" wait="true"  ]
[tb_show_message_window  ]
[anim name=role_button opacity=255 time=300 ]

#工場長
この前、飴の大きさが違うというクレームがあってね。どのラインに問題があるのか調べて欲しいんだ。[p]

[tb_hide_message_window  ]
[chara_hide  name="dummy"  time="500"  wait="true"  ]
[bg  time="500"  method="crossfade"  storage="factory_bg2.png" wait="true" ]
[mtext name="mtext" text="問題のある製造ラインを探せ！" x=240 y=240 size=36 time=500 in_effect="fadeInLeft" in_delay=10 in_sync="false" out_effect="fadeOutRight" out_delay=10 out_sync="false"]
[jump  storage="factory.ks"  target="*choice-exp"  ]
[s  ]
...
//	$('#exp-list').removeClass('flip-left');
	...
	$('#difficulty_base').animate({'right':'0px'},300);
	$('#btn-wrapper').animate({'right':'0px'},300,'easeInOutCubic');
//	$('#exp-result-expand-btn').animate({'right':'-1000px'}, 300);
...
[if exp="f.sim.state.answer[f.selected]==true"]
	...
	[glink  name="sys-btn"  storage="factory.ks"  size="20" width="100px" text="もう1回"  x="271"  y="480"  target="*back-top"  ]
	[glink  name="sys-btn" storage="factory.ks"  size="20"  width="100px" text="タイトルヘ"  x="555"  y="480"  target="*backtitle"  ]
